...
<div id="user_page">
	...
	<form action="" method="post" class="login_form" autocomplete="off">
		...
		<input type="password" class="login_input" name="password1" id="password1" pattern="^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?!.*\s).*$" autocomplete="current-password" placeholder="Enter a new password (more than 8 characters)">
		<p class="user_field">Type your new password again:</p>
		<input type="password" class="login_input" name="password2" id="password2" pattern="^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?!.*\s).*$" autocomplete="current-password" placeholder="Enter your new password again" onkeyup="passCheck(); return false;">
