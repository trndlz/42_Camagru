...
class commentsManager extends dbConfig {
	...
	public function getAllComments($id_photo) {
		$db = parent::dbConnect();
		$rows = $db->prepare('SELECT users.login, comments.date, comments.comment FROM comments INNER JOIN users ON comments.id_user = users.id_user WHERE comments.id_photo = ? ORDER BY comments.date DESC LIMIT 20');
